...
void LLadicionar(char *nome, int registro) {
    ...
    if (LLregInicio == NULL) {
        ...
        if (strcmp(LLnovo->nome, LLregInicio->nome) < 0) {
            ...
            if (LLaux->proximo == NULL) { // Inserir após LLaux (que é o último nó)
                LLaux->proximo = LLnovo;
                LLnovo->anterior = LLaux;
                LLregFim = LLnovo;
            } else { // Inserir LLnovo antes de LLaux->proximo
...
static void LLexcluirNoMeio_internal() {
    if (LLaux == NULL || LLaux->anterior == NULL || LLaux->proximo == NULL) return;
